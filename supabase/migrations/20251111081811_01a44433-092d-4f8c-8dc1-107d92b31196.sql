-- Update notify_on_order_change to include recipient's name in messages
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    ELSIF NEW.status = 'completed' AND accountant_id IS NOT NULL THEN
      PERFORM public.send_whatsapp_notification(
        accountant_id,
        'Hi ' || accountant_name || ', print job completed for order: ' || order_title || '. Ready for final processing.',
        NEW.id
      );
    END IF;
  END IF;
