-- Fix duplicate notifications by adding deduplication
-- Drop the existing trigger that calls edge function immediately
DROP TRIGGER IF EXISTS trigger_notify_new_notification ON public.notifications;

-- Update the notify_on_order_change function to prevent duplicate notifications
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    IF accountant_id IS NOT NULL THEN
      -- Check if notification already exists for this order and recipient
  ...
  ELSIF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    ELSIF NEW.status IN ('awaiting_accounting_approval', 'ready_for_print') AND accountant_id IS NOT NULL THEN
      SELECT COUNT(*) INTO existing_notification_count
      FROM public.notifications
      WHERE order_id = NEW.id 
      AND recipient_id = accountant_id
    ...
    ELSIF NEW.status = 'printing' THEN
      ...
      IF print_operator_id IS NOT NULL THEN
        ...
        FROM public.notifications
        WHERE order_id = NEW.id 
        AND recipient_id = print_operator_id
        AND message LIKE '%new print job ready%'
        AND created_at > NOW() - INTERVAL '1 minute';
